...
import { BookOpen, Search, XCircle, ChevronLeft, ChevronRight, FileText, FileSpreadsheet } from 'lucide-react';
...
export default function JurnalReport({
    journals,
    classes,
    mapels,
    gurus,
    ...
    const buildExportUrl = (type: 'pdf' | 'excel') => {
        const base = type === 'pdf'
            ? '/admin/jurnal-mengajar/export/pdf'
            : '/admin/jurnal-mengajar/export/excel';
    ...
    return (
        <div className="space-y-5 animate-fade-in text-left">
            <Head title="Jurnal Mengajar Guru" />
            ...
            <div className="border border-slate-200 bg-white px-6 py-5 dark:border-zinc-800 dark:bg-zinc-900">
                ...
                <p className="mt-1 max-w-2xl text-xs leading-relaxed text-slate-500 dark:text-neutral-500">
                    Monitor materi pembelajaran yang telah diajarkan guru pengajar di setiap kelas dan sesi mata pelajaran secara berkala.
                </p>
            </div>
            ...
            <div className="border border-slate-200 bg-white dark:border-zinc-800 dark:bg-zinc-900">
                <form onSubmit={handleFilter}>
                    {/* Filter fields */}
                    <div className="grid grid-cols-1 gap-3 p-5 sm:grid-cols-2 md:grid-cols-5">
                        {/* Kelas */}
                        <div>
                    ...
                    <div className="flex flex-col gap-2 border-t border-slate-100 px-5 py-3 sm:flex-row sm:items-center sm:justify-between dark:border-zinc-800">
                        ...
                        <div className="flex gap-2">
                            ...
                            >
                                <Search className="mr-1.5 size-3.5" /> Cari Jurnal
                            </Button>
                        </div>
                    </div>
                </form>
                ...
                <table className="w-full text-left text-sm table-fixed">
                    ...
                    <tbody>
                        {journals.data.length > 0 ? (
                            journals.data.map((j) => {
                                const { day, weekday } = formatDate(j.tanggal);
                                return (
                                    <tr
                                        key={j.id}
                                    ...
                                    >
                                        ...
                                        <td className="py-3.5 pr-5 pl-4 align-top">
                                            ...
                                            {j.catatan && (
                                                <p className="mt-1.5 text-[11px] leading-relaxed text-slate-500 dark:text-neutral-500 pl-2.5 border-l-2 border-slate-200 dark:border-zinc-700">
                                                    {j.catatan}
                                                </p>
                                            )}
                                        </td>
                                ...
                                <td colSpan={5} className="py-16 text-center">
                                    <div className="flex flex-col items-center justify-center gap-3">
                                        <div className="bg-slate-100 dark:bg-zinc-800 p-4">
                                            <BookOpen className="size-7 text-slate-400 dark:text-neutral-500" />
                                        </div>
                                        <div>
                                            <p className="text-xs font-semibold text-slate-800 dark:text-neutral-300">
                                                Jurnal Mengajar Kosong
                                            </p>
                ...
                <div className="flex items-center justify-between px-1 py-2">
                    ...
                    <div className="flex items-center gap-1.5">
                        <Button
                            ...
                            onClick={() =>
                                router.get(journals.prev_page_url || '', {}, { preserveScroll: true })
                            }
                        ...
                        <Button
                            ...
                            onClick={() =>
                                router.get(journals.next_page_url || '', {}, { preserveScroll: true })
                            }
                            className="h-8 rounded-sm text-[11px] font-medium cursor-pointer border-slate-200 hover:bg-slate-50 dark:border-zinc-700 dark:hover:bg-zinc-800 disabled:opacity-40"
                        >
...
JurnalReport.layout = {
    breadcrumbs: [
        ...
        { title: 'Jurnal Mengajar', href: '/admin/jurnal' },
    ],
};