    ...
    return (
        <div className="space-y-6 animate-fade-in text-left">
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left text-sm">
                            <thead className="sticky top-0 z-10 border-b border-neutral-100 bg-neutral-50 dark:border-zinc-800 dark:bg-zinc-900">
                                <tr>
                                    <th className="px-6 py-3.5 text-[10px] font-black tracking-widest text-neutral-400 uppercase dark:text-neutral-500">Nama Wali Murid</th>
                                    <th className="px-6 py-3.5 text-[10px] font-black tracking-widest text-neutral-400 uppercase dark:text-neutral-500">Kontak</th>
                                    <th className="px-6 py-3.5 text-[10px] font-black tracking-widest text-neutral-400 uppercase dark:text-neutral-500">Anak Terhubung</th>
                            ...
                            <tbody className="divide-y divide-neutral-100 dark:divide-zinc-800/60">
                                {filteredParents.length > 0 ? (
                                    filteredParents.map((p) => (
                                        <tr key={p.id} className="transition-colors hover:bg-violet-50/20 dark:hover:bg-violet-950/10">
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-3">
                                                    {p.foto ? (
                                                        <img src={p.foto} alt={p.name} className="size-11 rounded-2xl object-cover ring-2 ring-neutral-100 dark:ring-zinc-800" />
                                                    ) : (
                                                    ...
                                                    <div>
                                                        <p className="font-black text-neutral-900 dark:text-neutral-100">{p.name}</p>
                                                        <p className="text-[11px] text-neutral-400 dark:text-neutral-500">{p.jenis_kelamin === 'L' ? 'Bapak' : 'Ibu'}</p>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4">
                                            ...
                                            <td className="px-6 py-4">
                                                {p.anak.length > 0 ? (
                                                    ...
                                                    <span className="text-xs text-neutral-400 dark:text-neutral-500">Belum terhubung</span>
                                                )}
                                            </td>
                                            <td className="px-6 py-4">
                                                <div className="flex justify-end gap-1.5">
                                                    <Button size="sm" variant="outline" className="h-8 w-8 rounded-xl hover:bg-neutral-100 dark:hover:bg-zinc-800 cursor-pointer" onClick={() => openEditModal(p)}>
                                        ...
                                        <td colSpan={4} className="py-16 text-center">
                                            <div className="flex flex-col items-center gap-3">
                                                <div className="rounded-2xl bg-neutral-100 p-4 dark:bg-zinc-800">
                                                    <UsersRound className="size-8 text-neutral-400 dark:text-neutral-500" />
                                                </div>
                                                <div>
                                                    <p className="text-sm font-black text-neutral-700 dark:text-neutral-300">{searchQuery ? 'Orang Tua Tidak Ditemukan' : 'Belum Ada Orang Tua'}</p>
                                                    <p className="text-xs text-neutral-400 dark:text-neutral-500">{searchQuery ? `Tidak ada hasil untuk "${searchQuery}".` : 'Silakan tambahkan data orang tua baru.'}</p>
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
            ...
            <div className="space-y-3 md:hidden">
                {filteredParents.length > 0 ? (
                    filteredParents.map((p) => (
                        <div key={p.id} className="rounded-3xl border border-neutral-200/60 bg-white p-5 shadow-sm transition-all duration-200 hover:border-violet-200 hover:shadow-md dark:border-zinc-800/80 dark:bg-zinc-900/50">
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-3 min-w-0">
                                    {p.foto ? (
                                        <img src={p.foto} alt={p.name} className="size-14 rounded-2xl object-cover ring-2 ring-neutral-100 dark:ring-zinc-800 shrink-0" />
                                    ) : (
                                    ...
                                    <div className="min-w-0">
                                        <h3 className="truncate text-sm font-black text-neutral-900 dark:text-neutral-50">{p.name}</h3>
                                        <p className="text-[11px] text-neutral-400 dark:text-neutral-500">{p.jenis_kelamin === 'L' ? 'Bapak' : 'Ibu'}</p>
                                    </div>
                                </div>
                            ...
                            <div className="mt-3 space-y-1.5 border-t border-neutral-100 pt-3 dark:border-zinc-800/60">
                            ...
                            <div className="mt-3 border-t border-neutral-100 pt-3 dark:border-zinc-800/60">
                                <p className="mb-1.5 text-[9px] font-black uppercase tracking-widest text-neutral-400">Anak Terhubung</p>
                                {p.anak.length > 0 ? (
                                    <div className="flex flex-wrap gap-1.5">
                                        {p.anak.map((a) => (
                                    ...
                                    <span className="text-[11px] text-neutral-400 dark:text-neutral-500">Belum terhubung ke siswa</span>
                                )}
                            </div>
                        </div>
                    ))
                    ...
                    <div className="flex flex-col items-center justify-center gap-3 rounded-3xl border border-neutral-100 bg-white py-14 text-center dark:border-zinc-800 dark:bg-zinc-900/40">
                        ...
                        <div>
                            <p className="text-xs font-black text-neutral-800 dark:text-neutral-200">{searchQuery ? 'Orang Tua Tidak Ditemukan' : 'Belum Ada Orang Tua'}</p>
                            <p className="mt-0.5 text-[10px] text-neutral-400 dark:text-neutral-500">{searchQuery ? `Tidak ada hasil untuk "${searchQuery}".` : 'Silakan tambahkan data orang tua baru.'}</p>
                        </div>
                    </div>
                )}
            </div>
