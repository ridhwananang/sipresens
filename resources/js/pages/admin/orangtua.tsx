                ...
                <div className="relative flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div className="flex flex-wrap gap-2 sm:flex-col sm:items-end">
                        <span className="inline-flex items-center gap-1.5 rounded-2xl border border-neutral-200/60 bg-white/80 px-3 py-1.5 text-[11px] font-black text-neutral-700 shadow-xs dark:border-zinc-800/60 dark:bg-zinc-900/60 dark:text-neutral-300">
                            <UsersRound className="size-3.5 text-violet-500" />
                            {parents.length} Orang Tua Terdaftar
                        </span>
                    </div>
                </div>
            </div>
                        ...
                        <table className="w-full text-left text-sm text-neutral-500 dark:text-neutral-400">
                            <thead className="bg-neutral-50 text-[10px] font-black tracking-widest text-neutral-400 uppercase dark:bg-zinc-900/60 dark:text-neutral-500 border-b border-neutral-100 dark:border-neutral-900">
                                <tr>
                                    ...
                                    <th className="px-6 py-4">Anak Terhubung</th>
                            ...
                            <tbody className="divide-y divide-neutral-100 dark:divide-neutral-800/60">
                                {filteredParents.length > 0 ? (
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-3">
                                                    {p.foto ? (
                                                        ...
                                                        <span className={`flex size-10 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br ${avatarColor(p.id, p.jenis_kelamin)} text-sm font-black text-white shadow-sm`}>
                                                            {getInitials(p.name)}
                                                        </span>
                                                    )}
                                                    <div>
                                                    ...
                                                    <div className="flex flex-wrap gap-1.5">
                                                        {p.anak.map((a) => (
                                                            <span
                                                                key={a.id}
                                                                title={`NISN: ${a.nisn}`}
                                                                className="inline-flex items-center gap-1 rounded-full bg-indigo-50 px-2.5 py-0.5 text-[10px] font-bold text-indigo-700 dark:bg-indigo-950/20 dark:text-indigo-400"
                                                            >
                                                                <User className="size-2.5" />
                                                                {a.name}
                                                                <span className="rounded-full bg-indigo-100 px-1 text-[9px] font-black dark:bg-indigo-950/40">
                                                                    {a.kelas}
                                                                </span>
                                                            </span>
                                                        ))}
                                    ...
                                    <tr>
                                        <td colSpan={4} className="py-12 text-center text-neutral-450 dark:text-neutral-500 font-medium">
                                            {searchQuery
                                                ? `Tidak ada orang tua yang cocok dengan "${searchQuery}".`
                ...
                {filteredParents.length > 0 ? (
                    filteredParents.map((p) => (
                        ...
                        >
                            ...
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-3 min-w-0">
                                    {p.foto ? (
                                        ...
                                        <span className={`flex size-12 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br ${avatarColor(p.id, p.jenis_kelamin)} text-base font-black text-white shadow-sm`}>
                                            {getInitials(p.name)}
                                        </span>
                                    )}
                            ...
                            {/* Contact info */}
                            <div className="mt-3 grid grid-cols-1 gap-1.5 border-t border-neutral-100 pt-3 dark:border-zinc-800/60">
                                    ...
                                    <div className="flex flex-wrap gap-1.5">
                                        {p.anak.map((a) => (
                                            <span
                                                key={a.id}
                                                title={`NISN: ${a.nisn}`}
                                                className="inline-flex items-center gap-1 rounded-full bg-indigo-50 px-2.5 py-1 text-[10px] font-bold text-indigo-700 dark:bg-indigo-950/20 dark:text-indigo-400"
                                            >
                                                <User className="size-2.5" />
                                                {a.name}
                                                <span className="rounded-full bg-indigo-100 px-1.5 py-0.5 text-[9px] font-black dark:bg-indigo-950/40">
                                                    {a.kelas}
                                                </span>
                                            </span>
                                        ))}
                    ...
                    <div className="flex flex-col items-center justify-center space-y-3 rounded-3xl border border-neutral-100 bg-white py-12 text-center dark:border-zinc-900 dark:bg-zinc-900/40">
                        <div className="rounded-2xl bg-neutral-100 p-3 dark:bg-zinc-900">
                            <UsersRound className="size-8 text-neutral-400 dark:text-neutral-600" />
                        </div>
                        <div className="space-y-1">
                            <p className="text-xs font-black text-neutral-800 dark:text-neutral-200">
                                {searchQuery ? 'Orang Tua Tidak Ditemukan' : 'Belum Ada Orang Tua Terdaftar'}
                            </p>
                            <p className="mx-auto max-w-xs text-[10px] text-neutral-450 dark:text-neutral-500">
                                {searchQuery ? `Tidak ada hasil untuk "${searchQuery}".` : 'Silakan tambahkan data orang tua baru.'}
                            </p>
                        </div>
                    </div>
                )}
