...
export default function OrangTuaModal({ isOpen, onClose, editItem, students = [] }: OrangTuaModalProps) {
    ...
    useEffect(() => {
        if (editItem) {
            setData({
                ...
                siswa_ids: editItem.anak ? editItem.anak.map((a: any) => a.id) : [],
            });
        } else {
            reset();
        }
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        const url = editItem ? `/admin/orangtua/${editItem.id}` : '/admin/orangtua';
            ...
            <Card className="w-full max-w-2xl transform rounded-2xl border border-neutral-200 bg-white shadow-2xl transition-all dark:border-neutral-800 dark:bg-neutral-950">
                <CardHeader>
                    <CardTitle className="text-xl font-black text-neutral-900 dark:text-neutral-50">
                        {editItem ? 'Ubah Data Orang Tua' : 'Tambah Data Orang Tua'}
                    </CardTitle>
                </CardHeader>
                                    ...
                                    <div className="max-h-48 overflow-y-auto pr-1 space-y-1.5 scrollbar-thin">
                                        {filteredStudents.length > 0 ? (
                                            filteredStudents.map((siswa) => {
                                                ...
                                                const isAssignedToOther = siswa.orangtua_id && (!editItem || siswa.orangtua_id !== editItem.id);
                                                
                                                return (
                                                    <label
                                                        key={siswa.id}
                                                        className={`flex items-start gap-3 rounded-md p-2 transition-all cursor-pointer text-sm
                                                            ${isChecked 
                                                                ? 'bg-indigo-50/60 border border-indigo-100 dark:bg-indigo-950/10 dark:border-indigo-950/30' 
                                                            ...
                                                            <div className="mt-0.5 flex items-center gap-2 text-xs text-neutral-400">
                                                                ...
                                                                {isAssignedToOther && (
                                                                    <span className="font-semibold text-amber-600 dark:text-amber-500">
                                                                        (Punya wali lain, akan dipindahkan)
                                                                    </span>
                                                                )}
                                                            </div>
                                                        </div>
